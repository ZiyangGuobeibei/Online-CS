...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        elif self.armor > 0 and destination is not None and destination is not colony.beehive:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        cur = self.place
        cur_dis = 0
        while cur_dis < self.min_range or (len(cur.bees) == 0 and not cur == beehive and not cur == None and cur_dis <= self.max_range):
            cur = cur.entrance
            cur_dis += 1

        if cur is None or cur == beehive or cur_dis > self.max_range:
            return None
        else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        bonus = 0 if amount < self.armor else self.damage
        copy = self.place.bees[:]
        for bee in copy:
            bee.reduce_armor(amount + bonus)
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        return (not other.is_container) and (self.contained_ant is None)
        # END Problem 9
...
# END Problem 13
        ...
        if self.is_queen:
            ...
            while cur is not None:
                if cur.ant is not None and cur.ant is not self:
                    self.double_damage(cur.ant)
                    if cur.ant.is_container:
                        self.double_damage(cur.ant.contained_ant)
                cur = cur.exit
        else:
    ...
    def double_damage(self, ant):
        if ant is not None and ant is not self:
            if ant not in self.ants_buffed:
                ant.damage*=2
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.is_queen and amount >= self.armor:
            Insect.reduce_armor(self, amount)
            bees_win()
        else:
            Insect.reduce_armor(self, amount)
