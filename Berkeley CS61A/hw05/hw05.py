...
class VendingMachine:
    ...
    def vend(self):
        if self.balance >= self.price and self.stock:
            ...
            self.stock -= 1
            if change:
                return 'Here is your ' + self.name + ' and ${} change.'.format(change)
            else:
    ...
    def restock(self, num):
        self.stock+=num
        return 'Current ' + self.name + ' stock: {}'.format(self.stock)
...
def generate_paths(t, x):
    ...
    for b in t.branches:
        for list in generate_paths(b, x):
            yield [t.label] + list
    if t.label == x:
        yield [x]
