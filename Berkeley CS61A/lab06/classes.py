...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        assert len(opponent.deck.cards) >= 3 and len(opponent.hand) >= 3
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        opponent.deck.cards = [card for card in copy if (not card.attack == other_card.attack) and (not card.defense == other_card.defense)]
