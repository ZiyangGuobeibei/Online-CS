...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        left_max = find_max(t.branches[0])
        right_min = find_min(t.branches[1])
        return left_max <= t.label <= right_min and is_bst(t.branches[0]) and is_bst(t.branches[1])
    else:
        max = find_max(t.branches[0])
        min = find_min(t.branches[0])
        return (min >= t.label or max <= t.label) and is_bst(t.branches[0])

def find_min(t):
    if t.is_leaf():
        return t.label
    else:
        return min([t.label] + [find_min(b) for b in t.branches])

def find_max(t):
    if t.is_leaf():
        return t.label
    else:
        return max([t.label] + [find_max(b) for b in t.branches])
