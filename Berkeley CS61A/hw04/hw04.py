...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert is_weight(m) or is_mobile(m)
...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old:
        ...
        return tree(label(t), branches = [replace_leaf(b, old, new) for b in branches(t)])
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, password):
        nonlocal key, balance, wrongs
        if len(wrongs) == 3:
            return "Your account is locked. Attempts: {}".format(wrongs)
        if password != key:
            wrongs.append(password)
...
def make_joint(withdraw, old_password, new_password):
    ...
    old_key = old_password
    new_key = new_password
    cur_account = withdraw

    t = cur_account(0, old_key)
    if type(t) == str:
        return t
    else:
        def joint(amount, password):
            nonlocal old_key, new_key, cur_account
            if password != new_key:
                return cur_account(amount, password)
            else:
                return cur_account(amount, old_key)
        return joint
...
def quadratic(x, a, b, c):
    ...
    """
    "*** YOUR CODE HERE ***"
    #I can only use the reference to x once
    u = lower_bound(x)
    v = upper_bound(x)
    m = a * u + b
    n = a * v + b
    ...
    if a == 0:
        bias = interval(c, c)
        mult = interval(b, b)
        return add_interval(mul_interval(mult, x), bias)
    elif a < 0:
        if u >= -b / (2 * a) or v <= -b / (2 * a):
            return interval(h1, h2)
        else:
            return interval(c - b ** 2 / (4 * a), min(h1, h2))
    else:
        if u >= -b / (2 * a) or v <= -b / (2 * a):
            return interval(h1, h2)
        else:
            return interval(c - b ** 2 / (4 * a), max(h1, h2))